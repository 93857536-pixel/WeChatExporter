...
### Added
- **时间范围导出**：全部 / 最近 7/30/90 天 / 1 年 / 自定义
- **消息类型过滤**：文字、图片、语音、视频、表情、链接/文件、系统
- **导出预览**：导出前估算消息数、媒体数与体积
- **增量续导**：记住每会话上次导出时间，只拉新消息
- **Markdown / PDF 输出**：在 HTML、分类文件夹之外新增两种归档格式
- **群成员昵称映射**：群聊导出时尽量显示备注/群昵称
- **联系人收藏**：星标置顶常用会话
- **更细进度 + 重试失败**：单会话内分步进度；失败项可一键重试
- **可选语音转文字**（macOS Speech；Windows 可开关，暂为兼容占位）
- **一键环境检测**：doctor / 权限 / ffmpeg / 数据就绪状态

### Changed
- 准备数据优先尝试增量解密，失败再全量解密
- 版本升至 2.9.0

## [2.8.0] - 2026-07-24

### Added
- **设置中心**：导出类型、媒体选项、导出目录、浅色/深色主题等统一放入「设置」
- 界面主题：跟随系统 / 浅色 / 深色（双端）
- 设置页与主界面底部标注：`@林琝淏科技集团有限公司出品`
- 分类文件夹可选择是否附带 CSV / JSON；可开关表情包画廊与导出后自动打开目录

### Changed
- 主界面精简为会话选择与导出操作，详细选项改到设置中配置

## [2.7.0] - 2026-07-24

### Added
- **双导出方式可选**：
  - **单文件 HTML**（默认）：文字与媒体全部内嵌，浏览器直接打开
  - **分类文件夹**：每个会话生成独立目录，包含 `文字记录.txt`、`图片/`、`音频/`（优先 mp3）、`视频/`（优先 mp4）、`表情/` 与 `导出说明.txt`
- 分类文件夹模式自动导出媒体；若系统有 ffmpeg，会尽量把语音转成 mp3、视频转成 mp4

## [2.6.5] - 2026-07-24

### Fixed
- **导出串会话 / 反复失败**：导出查询改为优先使用唯一 `wxid` / `@chatroom`，不再用显示名模糊匹配（重名或同名联系人时会导出错人或持续报错）
- 导出结果会校验会话 ID，若与所选联系人不一致则明确报错，避免静默导出错人聊天记录
