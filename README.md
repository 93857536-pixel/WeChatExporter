...
- 图形界面：搜索、多选联系人/群聊
- **设置中心**：导出类型、媒体选项、浅色/深色主题等统一配置
- **高级导出**：时间范围、消息类型过滤、导出预览、增量续导、Markdown/PDF、群昵称映射、收藏夹、失败重试、环境检测
- **内置 wx-cli**：安装即用，无需单独安装命令行工具
- **就绪状态提示**：界面顶部显示当前进度（是否已完成「准备数据」）
- **单文件导出**：每个会话生成一个 `.html` 文件，文字与媒体（图片、表情、语音、视频）全部内嵌，浏览器直接打开
- **分类文件夹导出**：每个会话一个文件夹，文字集中在文档中，图片/音频(mp3)/视频(mp4)/表情分目录存放
- **可选媒体导出**：单文件模式下可勾选将媒体 base64 写入 HTML，并额外导出全部表情包画廊；WXGF 图片会自动尝试转码后显示（体积更大，耗时更长）
- 自动检测微信数据目录
